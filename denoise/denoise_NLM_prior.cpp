...
int main(int argc, char** argv){
	...
	desc.add_options()
			...
			("prior,p", po::value<string>(), "Prior filename")
	...
	auto prior = cuNDArray<float>(*read_nd_array<float>(vm["prior"].as<string>().c_str()));

	float* input_ptr = input.get_data_ptr();
	for (int i = 0; i < input.get_size(3); i++) {
		cuNDArray<float> out_view(prior.get_dimensions(),input_ptr);
		auto in_view = out_view;
		nonlocal_means_ref(&in_view,&out_view, &prior, noise);
		input_ptr += in_view.get_number_of_elements();
	}
