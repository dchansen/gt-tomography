...
int main(int argc, char** argv)
{
  ...
  parms.add_parameter( 'K', COMMAND_LINE_FLOAT,  1, "Kappa", true, "0.0" );
  ...
  *_precon_weights += *R_diag;
  R_diag.reset();
  R->set_codomain_dimensions(reg_image.get_dimensions().get());
  R->set_domain_dimensions(reg_image.get_dimensions().get());
  ...
  // Add "TV" regularization
   ...
   if( (alpha>0.0f) && (lambda>0.0f)){
     ...
     boost::shared_ptr<cuNDArray<_complext> > reg_image2 = expand( &reg_image, frames_per_reconstruction );
     PICS->set_prior(reg_image2);
     cg.add_nonlinear_operator(PICS);
   }

   {
  	 boost::shared_ptr< hoNDArray<_real> > host_norm = abs(&reg_image)->to_host();
  	   write_nd_array<_real>( host_norm.get(), "prior.real" );
   }
