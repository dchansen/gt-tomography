...
namespace Gadgetron{
template <class ARRAY_TYPE> class osMOMSolverD2 : public solver< ARRAY_TYPE,ARRAY_TYPE> {
	typedef typename ARRAY_TYPE::element_type ELEMENT_TYPE;
	typedef typename realType<ELEMENT_TYPE>::Type REAL;
public:
	osMOMSolverD2() :solver< ARRAY_TYPE,ARRAY_TYPE>() {
		_iterations=10;
		_beta = REAL(1);
		_alpha = 0.2;
		_gamma = 0;
		...
		for (int i =0; i < _iterations; i++){
			for (int isubset = 0; isubset < this->encoding_operator_->get_number_of_subsets(); isubset++){
				...
				*tmp_projections[subset] *= ELEMENT_TYPE(-1);
				...
				this->encoding_operator_->mult_MH(tmp_projections[subset].get(),x,subset,false);
				*x *= *precon_image;

				axpy(REAL(this->encoding_operator_->get_number_of_subsets()),x,z);


				//axpy(t*this->encoding_operator_->get_number_of_subsets(),x,&v);
