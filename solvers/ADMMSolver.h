...
namespace Gadgetron{
template <class ARRAY_TYPE> class ADMMSolver : public solver< ARRAY_TYPE,ARRAY_TYPE> {
	typedef typename ARRAY_TYPE::element_type ELEMENT_TYPE;
	typedef typename realType<ELEMENT_TYPE>::Type REAL;
public:
	ADMMSolver() :solver< ARRAY_TYPE,ARRAY_TYPE>() {
		_iterations=10;
	...
	boost::shared_ptr<ARRAY_TYPE> solve(ARRAY_TYPE* in){
		...
		ARRAY_TYPE u(*image_dims);
		clear(&u);
		ARRAY_TYPE d(*image_dims);
		clear(&d);
			...
			for (int isubset = 0; isubset < this->encoding_operator_->get_number_of_subsets(); isubset++){
				...
				*tmp_projections[subset] *= ELEMENT_TYPE(-1);
...

				{
					...
					axpy(_mu,&d,&tmp_image);
					axpy(-_mu,z,&tmp_image);
					axpy(_mu,&u,&tmp_image);
					*z += tmp_image;
				}
...
protected:
	void denoise(ARRAY_TYPE& u, ARRAY_TYPE& z, ARRAY_TYPE& d ){
