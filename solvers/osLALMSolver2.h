...
namespace Gadgetron{
template <class ARRAY_TYPE> class osLALMSolver2 : public solver< ARRAY_TYPE,ARRAY_TYPE> {
	typedef typename ARRAY_TYPE::element_type ELEMENT_TYPE;
	typedef typename realType<ELEMENT_TYPE>::Type REAL;
public:
	osLALMSolver2() :solver< ARRAY_TYPE,ARRAY_TYPE>() {
		_iterations=10;
		non_negativity_=false;
		inner_iterations=1;
		alpha = 0;
	}
	virtual ~osLALMSolver2(){};
	...
	boost::shared_ptr<ARRAY_TYPE> solve(ARRAY_TYPE* in){
		...
		boost::shared_ptr<ARRAY_TYPE> precon_image, diag;
		if (preconditioning_image_){
			precon_image = preconditioning_image_;
			diag = reciprocal(precon_image.get());
		}else {
		...
		nlcgSolver<ARRAY_TYPE> ncg;

		auto id_op = boost::make_shared<diagonalOperator<ARRAY_TYPE>>();
		//fill(diag.get(),ELEMENT_TYPE(1));
		id_op->set_diagonal(diag);

		id_op->set_domain_dimensions(image_dims.get());
		id_op->set_codomain_dimensions(image_dims.get());

		ncg.set_encoding_operator(id_op);
		ncg.set_max_iterations(1);
		ncg.set_non_negativity_constraint(non_negativity_);
		for (auto group : regularization_groups){
			for (auto op : group){
				ncg.add_regularization_group_operator(op);
			}
			ncg.add_group(1);
		}
		for (auto op : regularization_operators){
			ncg.add_regularization_operator(op,1);
		}
