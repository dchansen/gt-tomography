...
namespace Gadgetron{
template <class ARRAY_TYPE> class osMOMSolverL1 : public solver< ARRAY_TYPE,ARRAY_TYPE> {
	typedef typename ARRAY_TYPE::element_type ELEMENT_TYPE;
	typedef typename realType<ELEMENT_TYPE>::Type REAL;
public:
	osMOMSolverL1() :solver< ARRAY_TYPE,ARRAY_TYPE>() {
		_iterations=10;
		_beta = REAL(1);
		_alpha = 0.2;
		_gamma = 0;
	...
	boost::shared_ptr<ARRAY_TYPE> solve(ARRAY_TYPE* in){
		...
		ARRAY_TYPE d_projection(in->get_dimensions());
		clear(&d_projection);
		std::vector<boost::shared_ptr<ARRAY_TYPE> > d_projections = this->encoding_operator_->projection_subsets(&d_projection);

		ARRAY_TYPE u_projection(in->get_dimensions());
		u_projection = *in;
		std::vector<boost::shared_ptr<ARRAY_TYPE> > u_projections = this->encoding_operator_->projection_subsets(&u_projection);
		...
		for (int i =0; i < _iterations; i++){
			for (int isubset = 0; isubset < this->encoding_operator_->get_number_of_subsets(); isubset++){
				...
				*tmp_projections[subset] -= *u_projections[subset];
				//*tmp_projections[subset] -= *d_projections[subset];
				if( this->output_mode_ >= solver<ARRAY_TYPE,ARRAY_TYPE>::OUTPUT_VERBOSE ){
			...
			linearOperator<ARRAY_TYPE>* all = this->encoding_operator_.get();
			all->mult_M(z,&tmp_projection);

			u_projection = tmp_projection;
			//u_projection -= d_projection;
			shrink_diff(&u_projection,in,0.5f);
			d_projection -= tmp_projection;
			d_projection += u_projection;
			if (dump){
				std::stringstream ss;
				ss << "osMOM-" << i << ".real";
