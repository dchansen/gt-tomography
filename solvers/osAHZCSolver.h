...
namespace Gadgetron{
template <class ARRAY_TYPE> class osAHZCSolver : public solver< ARRAY_TYPE,ARRAY_TYPE> {
	typedef typename ARRAY_TYPE::element_type ELEMENT_TYPE;
	typedef typename realType<ELEMENT_TYPE>::Type REAL;
public:
	osAHZCSolver() :solver< ARRAY_TYPE,ARRAY_TYPE>() {
		_iterations=10;
		_beta = REAL(1);
		_alpha = 0.2;
		_gamma = 0;
	...
	boost::shared_ptr<ARRAY_TYPE> solve(ARRAY_TYPE* in){
		...
		bool has_regularization = !((regularization_operators.empty()) && (regularization_groups.empty()));
		...
		if (!has_regularization) gam = 1;

		std::vector<std::vector<ARRAY_TYPE>> reg_group_data;
		for( auto & reg_group :  regularization_groups){
			reg_group_data.push_back(std::vector<ARRAY_TYPE>());
			for( int k = 0; k < reg_group.size(); k++){
				reg_group_data.back().push_back(ARRAY_TYPE(reg_group[k]->get_domain_dimensions()));
				clear(&reg_group_data.back().back());
			}
			...
			for (int isubset = 0; isubset < this->encoding_operator_->get_number_of_subsets(); isubset++){
				...
				if (has_regularization) {
					...
					for (auto reg_op : regularization_operators){
						...
						reg_op->mult_MH(&data,&tmp_image,true);
					}*/

					for (int k = 0; k < regularization_groups.size(); k++){
						auto & reg_group = regularization_groups[k];
						auto & datas = reg_group_data[k];
						//auto datas = std::vector<ARRAY_TYPE>(reg_group.size());
						for (auto n = 0u; n < reg_group.size(); n++){
							//datas[n] = ARRAY_TYPE(reg_group[n]->get_codomain_dimensions());

							*x *= sigma;
							reg_group[n]->mult_M(x,&datas[n],true);
							*x /= sigma;
					...
					//updateFgroup is the resolvent operators on the group
						...
						for (auto i = 0u; i < reg_group.size(); i++){
							reg_group[i]->mult_MH(&datas[i],&tmp_image,true);
						}

					}
					tmp_image *= tau;
					axpy(REAL(-2),x,&tmp_image);
					tmp_image /= (2*tau*avg_lambda);
				}
				REAL stepsize = REAL(_beta/(1+_gamma*i));
				...
				*tmp_projections[subset] -= *subsets[subset];
				...
				if (has_regularization){
					*precon_image += REAL(1.0)/(2*tau*avg_lambda)-precon_mod;
					precon_mod = REAL(1.0)/(2*tau*avg_lambda);

				}

				if (has_regularization){
					tmp_image /= *precon_image;
					axpy(stepsize,&tmp_image,x);
				}


				this->encoding_operator_->mult_MH(tmp_projections[subset].get(),&tmp_image,subset,false);
				tmp_image /= *precon_image;
				axpy(-stepsize*this->encoding_operator_->get_number_of_subsets(),&tmp_image,x);
