...
namespace Gadgetron{
/** Nonlinear conjugate gradient solver. Corresponds to version 4.0 of the CG_DESCENT algorithm
 * Adapted from William W. Hager and Hongchao Zhang "THE LIMITED MEMORY CONJUGATE GRADIENT METHOD"
 *SIAM Journal on Optimization, 23 (2013), pp. 2150-2168.
 *
 */

template <class ARRAY_TYPE> class cgdescentSolver : public gpSolver<ARRAY_TYPE>
{
...
public:
	...
	cgdescentSolver(): gpSolver<ARRAY_TYPE>() {
		iterations_ = 10;
	...
	virtual boost::shared_ptr<ARRAY_TYPE> solve(ARRAY_TYPE* in)
		...
		if( this->encoding_operator_.get() == 0 ){
			throw std::runtime_error("Error: cgdescentSolver::compute_rhs : no encoding operator is set" );
		}
		...
		if( image_dims->size() == 0 ){
			throw std::runtime_error("Error: cgdescentSolver::compute_rhs : encoding operator has not set domain dimension" );
		}
		...
		for (int i = 0; i < iterations_; i++){
			...
			if (i == 0){
				...
				ELEMENT_TYPE dgold = dot(&d,&g_old);
				g_old -= g_step;
				REAL gg = real(dot(&g_step,&g_step));
				ELEMENT_TYPE dy = -dot(&d,&g_old);
				ELEMENT_TYPE gy = -dot(&g_step,&g_old);
				//ELEMENT_TYPE beta = -dot(g,g_old)/g_old_norm; //PRP ste[
				//ELEMENT_TYPE theta = gy/g_old_norm;

								ELEMENT_TYPE dg = dot(&d,&g_step);
				REAL betaCG = real(gy/dy-dot(&g_old,&g_old)/dy*dg/dy);
				REAL nabla = nabla0*(dgold/dot(&d,&d));
				REAL beta =std::max(betaCG,nabla);
				d *= beta;
...
protected:
	...
	public:
		...
		FunctionEstimator(ARRAY_TYPE* _encoding_space,ARRAY_TYPE* _encoding_step,std::vector<ARRAY_TYPE>* _regEnc,std::vector<ARRAY_TYPE>* _regEnc_step, ARRAY_TYPE * _x, ARRAY_TYPE * _d, ARRAY_TYPE * _g, ARRAY_TYPE * _g_step, cgdescentSolver<ARRAY_TYPE> * _parent)
	{
	...
	private:
		...
		cgdescentSolver<ARRAY_TYPE>* parent;
		ARRAY_TYPE xtmp;
