...
public:
	...
	cuATrousOperator() : invertibleOperator<cuNDArray<T>>() {
	...
	virtual void mult_M(cuNDArray<T>* in , cuNDArray<T>* out, bool accumulate=false){
		...
		for (int level = 0; level < max_levels; level++){
			auto wav = apply_wavelet(prev.get(),level);
...
protected:
	...
	boost::shared_ptr<cuNDArray<T>> apply_wavelet(cuNDArray<T>* in, int level){
		cuNDArray<T>* tmp1 = new cuNDArray<T>(*in);
		if (level == -1)
			return boost::shared_ptr<cuNDArray<T>>(tmp1);
		cuNDArray<T>* tmp2 = new cuNDArray<T>(in->get_dimensions());

		for (auto dim = 0; dim < levels.size(); dim++){
			if (levels[dim] > level){
				aTrousWavelet(tmp1,tmp2,&kernel,std::pow(2,level),dim,false);
				std::swap(tmp1,tmp2);
			}
		}
